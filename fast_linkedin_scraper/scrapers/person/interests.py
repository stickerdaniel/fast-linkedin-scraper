...
def scrape_interests(page: Page, person: Person) -> None:
    ...
    try:
        ...
        for list_container in list_containers:
            ...
            for item in items:
                try:
                    interest = _extract_interest_from_item(item)
                    if interest:
...
def _extract_interest_from_item(item: Locator) -> Optional[Interest]:
    ...
    try:
        ...
        try:
            ...
            if follower_elem.is_visible():
                followers_text = follower_elem.inner_text()
